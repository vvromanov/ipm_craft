...
void Resources::CollectResources(std::map<const Resource *, uint32_t>& have, std::map<const Resource *, uint32_t> &list, uint32_t count, double k) const {
    for (const auto &r: *this) {
...
static void DumpTime(uint32_t time) {
...
void Resources::DumpResources(std::map<const Resource *, uint32_t>& have, uint32_t count, double k) const {
    ...
    for (auto i: list) {
        if (i.first->getType() == rtItem) {
            uint32_t time = i.first->getTimeToProduce();
            uint32_t total_time = time * i.second;
